...
class TestSerialConfig:
    ...
    def test_to_serial_kwargs(self):
        ...
        """
        config = SerialConfig(port="COM1", baudrate=9600, timeout=1.0)
    ...
    @pytest.mark.parametrize(
        "port,baudrate,expected_port,expected_baudrate",
        [
            # 参数化测试：测试不同的端口和波特率组合
            ("COM1", 9600, "COM1", 9600),
    ...
    def test_serial_config_parametrized(
        self, port, baudrate, expected_port, expected_baudrate
    ):
        """
        参数化测试：测试不同的端口和波特率组合
...
class TestTransferConfig:
    ...
    @pytest.mark.parametrize(
        "max_data_length,request_timeout,retry_count,show_progress",
        [
            # 参数化测试：测试各种有效的参数组合
    ...
    def test_transfer_config_valid_combinations(
        self, max_data_length, request_timeout, retry_count, show_progress
    ):
        """
        参数化测试：验证各种有效的参数组合
...
class TestConfigIntegration:
    ...
    def test_configs_typical_usage(self):
        ...
        high_speed_serial = SerialConfig(
            port="COM1", baudrate=1728000, timeout=1.0  # 高波特率  # 短超时
        )
        ...
        reliable_serial = SerialConfig(
            port="COM1", baudrate=115200, timeout=5.0  # 稳定波特率  # 长超时
        )
...
class TestTransferConfigP1AChunkSize:
    """测试TransferConfig的P1-A块大小协商功能"""

    def test_default_chunk_size_behavior(self):
        """测试默认块大小行为"""
        config = TransferConfig()

        # 默认情况下，协商块大小为None
        assert config.negotiated_chunk_size is None

        # 有效块大小应该是默认的max_data_length
        assert config.get_effective_chunk_size() == config.max_data_length

    def test_update_chunk_size_valid(self):
        """测试有效的块大小更新"""
        config = TransferConfig()

        # 更新为有效块大小
        config.update_chunk_size(2048)
        assert config.negotiated_chunk_size == 2048
        assert config.get_effective_chunk_size() == 2048

    def test_update_chunk_size_boundaries(self):
        """测试块大小边界值"""
        config = TransferConfig()

        # 测试最小值
        config.update_chunk_size(512)  # MIN_CHUNK_SIZE
        assert config.negotiated_chunk_size == 512

        # 测试最大值
        config.update_chunk_size(16384)  # MAX_CHUNK_SIZE
        assert config.negotiated_chunk_size == 16384

    def test_update_chunk_size_invalid_too_small(self):
        """测试过小的块大小"""
        config = TransferConfig()

        with pytest.raises(ValueError, match="块大小必须在"):
            config.update_chunk_size(256)  # 小于MIN_CHUNK_SIZE

    def test_update_chunk_size_invalid_too_large(self):
        """测试过大的块大小"""
        config = TransferConfig()

        with pytest.raises(ValueError, match="块大小必须在"):
            config.update_chunk_size(32768)  # 大于MAX_CHUNK_SIZE

    def test_reset_chunk_size(self):
        """测试重置块大小"""
        config = TransferConfig()
        ...
        # 重置后应该回到默认值
        config.reset_chunk_size()
        assert config.negotiated_chunk_size is None
        assert config.get_effective_chunk_size() == config.max_data_length

    def test_chunk_size_override_default(self):
        """测试块大小覆盖默认值"""
        config = TransferConfig(max_data_length=1024)

        # 默认使用max_data_length
        assert config.get_effective_chunk_size() == 1024

        # 协商后使用新值
        config.update_chunk_size(4096)
        assert config.get_effective_chunk_size() == 4096
    ...
    def test_chunk_size_parametrized(self, chunk_size, expected):
        """参数化测试不同块大小"""
        config = TransferConfig()
        config.update_chunk_size(chunk_size)
        assert config.get_effective_chunk_size() == expected
