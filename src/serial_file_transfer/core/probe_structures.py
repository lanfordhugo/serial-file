...
class ProbeRequestData:
    ...
    def pack(self) -> bytes:
        """打包成字节数据"""
        return struct.pack('<IBI', self.device_id, self.protocol_version, self.random_seed)
...
class ProbeResponseData:
    ...
    def pack(self) -> bytes:
        """打包成字节数据"""
        # 头部：设备ID + 协议版本 + 随机数
        header = struct.pack('<IBI', self.device_id, self.protocol_version, self.random_seed)
    ...
    @classmethod  
    def create_response(cls, request: ProbeRequestData, 
                       supported_baudrates: List[int]) -> 'ProbeResponseData':
        """基于探测请求创建响应"""
        return cls(
...
class CapabilityNegoData:
    ...
    def unpack(cls, data: bytes) -> Optional['CapabilityNegoData']:
        """从字节数据解包"""
        try:
            if len(data) != 21:  # 4+1+4+8+4 = 21字节
                return None
            values = struct.unpack('<IBIQI', data)
            return cls(*values)
        except struct.error:
            return None
...
class CapabilityAckData:
    ...
    def pack(self) -> bytes:
        """打包成字节数据"""
        return struct.pack('<IB', self.session_id, self.accept_status)
    
    @classmethod
    def unpack(cls, data: bytes) -> Optional['CapabilityAckData']:
        """从字节数据解包"""
        try:
            if len(data) != 5:  # 4+1 = 5字节
                return None
            session_id, accept_status = struct.unpack('<IB', data)
            return cls(session_id, accept_status)
        except struct.error:
            return None
...
class SwitchBaudrateData:
    ...
    def pack(self) -> bytes:
        """打包成字节数据"""
        return struct.pack('<IIH', self.session_id, self.new_baudrate, self.switch_delay_ms)
