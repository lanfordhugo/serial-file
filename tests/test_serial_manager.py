...
class TestSerialManager:
    ...
    def test_open_success(self, mock_serial_class):
        ...
        # 创建管理器并打开串口（使用明确的超时值避免触发P1-B自适应优化）
        config = SerialConfig(port="COM1", baudrate=9600, timeout=0.2)
        manager = SerialManager(config)
...
class TestSerialManagerP1BOptimization:
    """测试P1-B优化功能"""

    def test_adaptive_timeout_calculation(self):
        """测试波特率自适应超时计算"""
        # 高波特率测试 (1 Mbps)
        config_high = SerialConfig(port="COM_MOCK", baudrate=1_000_000)
        manager_high = SerialManager(config_high)

        # 计算期望超时: max(0.05, (5 / 1000000) * 12) ≈ 0.05
        # FRAME_FORMAT_SIZE通常为5字节
        expected_timeout_high = max(0.05, (5 / 1_000_000) * 12)

        with patch("serial.Serial") as mock_serial:
            mock_port = MagicMock()
            mock_serial.return_value = mock_port

            result = manager_high.open()
            assert result is True

            # 验证超时参数被正确传递
            call_args = mock_serial.call_args[1]
            assert "timeout" in call_args
            assert abs(call_args["timeout"] - expected_timeout_high) < 0.001

    def test_adaptive_timeout_low_baudrate(self):
        """测试低波特率的自适应超时"""
        # 低波特率测试 (115200 bps)
        config_low = SerialConfig(port="COM_MOCK", baudrate=115_200)
        manager_low = SerialManager(config_low)

        # 计算期望超时: max(0.05, (5 / 115200) * 12) ≈ 0.0005 -> 0.05 (取最小值)
        expected_timeout_low = max(0.05, (5 / 115_200) * 12)

        with patch("serial.Serial") as mock_serial:
            mock_port = MagicMock()
            mock_serial.return_value = mock_port

            result = manager_low.open()
            assert result is True

            # 验证超时参数
            call_args = mock_serial.call_args[1]
            assert call_args["timeout"] >= 0.05  # 最小值保护

    def test_write_with_flush(self):
        """测试write方法包含flush调用"""
        config = SerialConfig(port="COM_MOCK")
        manager = SerialManager(config)

        with patch("serial.Serial") as mock_serial:
            mock_port = MagicMock()
            mock_port.is_open = True
            mock_port.write.return_value = 5  # 写入5字节
            mock_serial.return_value = mock_port
            ...
            mock_port.flush.assert_called_once()  # 验证flush被调用

    def test_write_partial_failure_no_flush(self):
        """测试部分写入失败时不调用flush"""
        config = SerialConfig(port="COM_MOCK")
        manager = SerialManager(config)
        ...
        with patch("serial.Serial") as mock_serial:
            mock_port = MagicMock()
            mock_port.is_open = True
            mock_port.write.return_value = 3  # 只写入3字节，但期望5字节
            mock_serial.return_value = mock_port
            ...
            assert result is False
            mock_port.write.assert_called_once_with(test_data)
            mock_port.flush.assert_not_called()  # 失败时不应调用flush

    def test_adaptive_timeout_various_baudrates(self):
        """测试不同波特率的超时计算"""
        test_cases = [
            (9600, max(0.05, (5 / 9600) * 12)),
            (115200, max(0.05, (5 / 115200) * 12)),
            (460800, max(0.05, (5 / 460800) * 12)),
            (1728000, max(0.05, (5 / 1728000) * 12)),
        ]

        for baudrate, expected_timeout in test_cases:
            config = SerialConfig(port="COM_MOCK", baudrate=baudrate)
            manager = SerialManager(config)
            ...
            with patch("serial.Serial") as mock_serial:
                ...
                call_args = mock_serial.call_args[1]
                actual_timeout = call_args["timeout"]

                # 验证超时计算正确
                assert (
                    abs(actual_timeout - expected_timeout) < 0.001
                ), f"波特率{baudrate}的超时计算错误: 期望{expected_timeout}, 实际{actual_timeout}"

                # 验证最小超时保护
                assert actual_timeout >= 0.05, f"波特率{baudrate}的超时低于最小值0.05s"
