...
class TestSmartCLI:
    ...
    def test_send_no_ports(
        self, mock_get_baudrate, mock_get_path, mock_get_port, mock_input
    ):
        """测试手动发送在没有串口时的行为"""
        # 模拟没有串口
        mock_get_port.return_value = None
    ...
    @patch("src.serial_file_transfer.cli.file_transfer.FileTransferCLI.get_baudrate")
    def test_receive_no_ports(
        self, mock_get_baudrate, mock_get_save_path, mock_get_port, mock_input
    ):
        """测试手动接收在没有串口时的行为"""
        # 模拟没有串口
        mock_get_port.return_value = None
