...
class TestFileSenderFileOperations:
    ...
    @pytest.mark.parametrize("file_size,addr,length,expected_length", [
        # 参数化测试：不同的文件大小和读取参数
        (100, 0, 10, 10),      # 正常读取
...
class TestFileSenderProtocol:
    ...
    def test_send_filename_unicode(self):
        ...
        with patch('serial_file_transfer.transfer.sender.FrameHandler') as mock_frame_handler:
            mock_frame_handler.pack_frame.return_value = b"packed_frame"
...
class TestFileSenderErrorHandling:
    ...
    def test_send_filename_with_exception(self):
        ...
        with patch('serial_file_transfer.transfer.sender.FrameHandler.pack_frame', side_effect=Exception("打包失败")):
            result = sender.send_filename("test.txt")
